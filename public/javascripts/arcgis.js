...
function load_arcgis(div) {
    require([ 'esri/map',
        ...
        office = new KMLLayer('http://127.0.0.1/kml/offices.kml');
        office.visible = false;
        map.addLayer(office);

        placeName = new KMLLayer('http://127.0.0.1/kml/placenames.kml');
        placeName.visible = false;
        map.addLayer(placeName);

        trail = new KMLLayer('http://127.0.0.1/kml/trails.kml');
        map.addLayer(trail);
    });
}
